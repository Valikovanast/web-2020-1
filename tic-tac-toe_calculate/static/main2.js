    ...
    for (char of str) {
        if (isDigit(char) || char == '.') {
            ...
            if (lastNumber.length > 0) {
                tokens.push(lastNumber);//push- добавляет в конец массива
                lastNumber = '';
            }
        }
        if (isOperation(char) || char == '(' || char == ')') {
            tokens.push(char);// если знак является оператором
        }
    }
    if (lastNumber.length > 0) {
...
function compile(str) {
    ...
    for (token of tokenize(str)) {
        if (isNumeric(token)) {
            ...
            while (stack.length > 0 && stack[stack.length - 1] != '(') {
                out.push(stack.pop());//pop- удаляет последний элемент массива и возвращает его
            }
            stack.pop();
        }
    }
    ...
    return out.join(' ');//объединяет все элементы массива в строку
}
...
function evaluate(str) {
    let ch = 0;
    let arr = [];
    for (out of tokenize(str)) {
        arr.push(out);
        if (arr[arr.length - 1] == '+') {
            ch = ((Number(arr[arr.length - 3])) + (Number(arr[arr.length - 2]))).toFixed(2);
            arr.splice(arr.length - 3, 3, ch);
        }
        if (arr[arr.length - 1] == '-') {
            ch = ((Number(arr[arr.length - 3])) - (Number(arr[arr.length - 2]))).toFixed(2);
            arr.splice(arr.length - 3, 3, ch);
...
function clickHandler(e) {
    ...
    if (e.target.classList.contains('digit') || e.target.classList.contains('operation') || e.target.classList.contains('bracket')) {
        screen.innerText += e.target.innerText;
    }
    if (e.target.classList.contains('result')) {
        str = compile(screen.innerHTML);
        screen.innerText = evaluate(str);
    }
